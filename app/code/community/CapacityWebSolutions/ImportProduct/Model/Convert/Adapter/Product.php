...
class CapacityWebSolutions_ImportProduct_Model_Convert_Adapter_Product extends Mage_Catalog_Model_Convert_Adapter_Product
{
    ...
    public function saveRow(array $importData)
    {
		...
		/* This Code is for set Base Image, Small Image and Thumbnail Image Added by Parag 09-10-12 */	
		/* This Code for get attribute id using attribute code */
		$image_id = Mage::getModel('catalog/product')->getResource()->getAttribute("image")->getData('attribute_id');
		$small_image_id = Mage::getModel('catalog/product')->getResource()->getAttribute("small_image")->getData('attribute_id');
		$thumbnail_id = Mage::getModel('catalog/product')->getResource()->getAttribute("thumbnail")->getData('attribute_id');
		/* End of Get Attribute Id Code */
		
		$resource = Mage::getSingleton('core/resource');
		$catalogMediaGallary = $resource->getTableName('catalog_product_entity_media_gallery');
		$catalogMediaGallaryValue = $resource->getTableName('catalog_product_entity_media_gallery_value');
		$catalogEntityVarchar = $resource->getTableName('catalog_product_entity_varchar');
		
		$resource = Mage::getSingleton('core/resource');
		$conn = $resource->getConnection('core_read');
		$conn->query("UPDATE ". $catalogMediaGallary." AS mg, ".
		$catalogMediaGallaryValue ." AS mgv,".
		$catalogEntityVarchar." AS ev
		SET ev.value = mg.value
		WHERE
		mg.value_id = mgv.value_id
		AND mg.entity_id = ev.entity_id
		AND ev.attribute_id IN ($image_id,$small_image_id,$thumbnail_id)");
		/* End of Image Set Code */
		...
		if(isset($importData['sku'])){
        	...
        	if (isset($custom_options)) {
				$gallery = $custom_options;
				//Get the images
				$galleryData = $product->getCustomAttribute();								
				foreach($galleryData['custom'] as $image){
				//If image exists
			    	if ($gallery->getBackend()->getCustomAttribute($product)) {
			            $gallery->getBackend()->removeCustomAttribute($product);
			        }
				}				
			}
        }*/
		...
		if(isset($custom_options) && $custom_flag=="Cws"){
